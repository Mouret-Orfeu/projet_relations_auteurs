 ...
 * 
 * - Le tableau des auteur_struct et celui des Article_struct seron stocké dans graphe_struct_Louis.
 * - Pour finir, un tableau de pointeur vers les auteurs_struct est crée, et rangé dans graphe_Orfeu (fonction dans *fonctions_garphe.c*). 
 ...
 * 
 * - Il ne reste plus qu'à appliquer la fonction do_Dijkstra() (*Dijkstra.c*) au graphe_Orfeu, avec 2 noms d'auteur de la base de donnée.
 ...
 * 
 * une structure finale  graphe_struct_Louis regroupant toutes les structures désérialisées/crées est générée pour simplifier les free et l'utilisation
 * 
 * \subsection Dijkstra
 * 
 * do_Dijkstra() s'applique sur un tableau de pointeur vers les auteurs_struct, situé dans graphe_struct_Orfeu. 
 ...
 * 
 *  Tu peux laisser cette phrase mais on est pas sensé raconter ce genre de truc dans le rapport je pense //(Louis:)j'ai pris l'habitude de profiler le programe au fure et à mesure de sont avancement pour mieux comprendre le fonctionement est les optimisations
 ...
 * 
 * @param chemins 
 * @return graphe_struct_Louis 
 */
graphe_struct_Louis gen_graph(listeChemin chemins)
{
    ...
    graphe_struct_Louis graph = deserialise_Graph(DBinverse, DBauteur, DBarticle);
 ...
 */
void find_all(char *search, graphe_struct_Louis *graph, int verbose)
{
...
//DEBUG
/**
 * @brief test des couples d'auteurs au hasard, jusqu'à en trouver un pour lequel il y a un chemin
 * 
 */
//void find_author_couple_with_path(graphe_struct_Louis *graph, tab_auteur_struct *tab_auteur)
//{
//    srand(time(NULL)); // Seed the random number generator
//
//    // Ensure there are authors to choose from
//    if(tab_auteur->nombre_auteur <= 0)
//    {
//        printf("No authors available.\n");
//        return;
//    }
//
//    // Continuously attempt to find a path between two random authors
//    while(1)
//    {
//        // Select two random authors
//        int index1 = rand() % tab_auteur->nombre_auteur;
//        int index2 = rand() % tab_auteur->nombre_auteur;
//
//        auteur_struct *auteur1 = &tab_auteur->tab_auteur[index1];
//        auteur_struct *auteur2 = &tab_auteur->tab_auteur[index2];
//
//        // Attempt to find a path between the authors
//        printf("Attempting to find a path between %s and %s...\n", auteur1->nom_auteur, auteur2->nom_auteur);
//        int path_found=djikstrasearch(auteur1->nom_auteur, auteur2->nom_auteur, *graph);
//
//        // If a path was found, exit the loop
//        if (path_found)
//        {
//            printf("Path found between: %s, %s\n", auteur1->nom_auteur, auteur2->nom_auteur);
//            break;
//        }
//    }
//}
 ...
 */
int djikstrasearch(char *a, char *b, graphe_struct_Louis graph)
{
    int path_found=0;

    graphe_struct_Orfeu graphe_Orfeu = faire_graphe_ptr_auteur(graph);
    plus_court_chemin_struct *plus_court_chemin_ptr = do_Dijkstra(graphe_Orfeu, a, b);
    ...
    free_Dijkstra(graphe_Orfeu, plus_court_chemin_ptr);
...
int main(int argc, char *argv[])
{
    ...
    graphe_struct_Louis graph = gen_graph(oui.chemais);


    //DEBUG
    //find_author_couple_with_path(&graph, &graph.tab_auteur_struct);
