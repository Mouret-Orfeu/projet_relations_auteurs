...
graphe_struct_Louis ggen_unwrap_Graph(){
    ...
    graphe_struct_Louis graph = deserialise_Graph(DBxml,DBinverse,DBarticle);
...
graphe_struct_Louis local_gen_Graph_from_XML(){
...
void local_serialise_Graph(){
    ...
    graphe_struct_Louis graph = local_gen_Graph_from_XML();
...
graphe_struct_Louis local_gen_custom_Graph_from_XML(){
...
listeFichier openDB(int type,int mode){
    ...
    case customdb:
        ...
        break;
    case orpheudb:
        listefile.XML               = fopen(orpheuxXML      ,"r");
        if(mode == ecriture){
 ...
 * @param nbtime 
 */
void testgraph(graphe_struct_Louis graphe_Louis,int nbtime){
    INFO("Test Graph")
    srand(time(NULL));
    ...
    for (int i = 0; i < nbtime; i++)
    {
        ...
        int a,b;
        a = rand()%graphe_Louis.tab_auteur_struct.nombre_auteur;
        b = rand()%graphe_Louis.tab_auteur_struct.nombre_auteur;
        char* nom_auteur_depart     = graphe_Orfeu.graphe[a]->nom_auteur;
        char* nom_auteur_destination= graphe_Orfeu.graphe[b]->nom_auteur;
        WARNING("depart\t: %s ",nom_auteur_depart)
        WARNING("arriver\t: %s",nom_auteur_destination)
        plus_court_chemin_struct* plus_court_chemin_ptr= do_Dijkstra(graphe_Orfeu, nom_auteur_depart, nom_auteur_destination);
...
void all(int mode ){
    ...
    graphe_struct_Louis legraphW = gen_Graph_from_XML(mesfichierWrite.XML);
    ...
    graphe_struct_Louis legraph =  deserialise_Graph(
...
int main(int argc, char const *argv[])
{
    ...
    if (argc >= 3)
    {
        ...
        if(strcmp("sg",compstr)==0){
            ...
            graphe_struct_Louis legraph = gen_Graph_from_XML(mesfichierWrite.XML);
            ...
            free_Graph_struct(legraph);
        STR("gxml")
            graphe_struct_Louis legraph = gen_Graph_from_XML(mesfichier.XML);
            free_Graph_struct(legraph);
        STR("dg")
            graphe_struct_Louis legraph =  deserialise_Graph(
                                        mesfichier.DBficheLecture,
